...
<!-- Pesan Sukses (Tetap di luar agar mudah terlihat dan tertutup) -->
@if(session('success'))
<div class="mb-6 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl relative flex items-center shadow-sm" role="alert">
    ...
    <span class="block sm:inline text-sm font-semibold">{{ session('success') }}</span>
</div>
@endif

<!-- ============================================== -->
<!-- HEADER PROYEK (Desain Menyatu & Elegan) -->
<!-- ============================================== -->
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-8 mb-8">
    ...
    <div class="flex flex-wrap items-center justify-between mb-6 pb-6 border-b border-slate-100 gap-4">
        ...
        <div class="flex items-center gap-2.5">
            <!-- Tombol Edit -->
            <button onclick="toggleModal('modalEditProyek')" class="inline-flex items-center px-3.5 py-1.5 bg-white border border-slate-200 text-blue-600 hover:bg-blue-50 hover:border-blue-200 text-sm font-bold rounded-xl transition-colors shadow-sm">
            ...
            <form id="form-hapus-proyek" action="{{ route('proyek.destroy', $proyek->id) }}" method="POST" class="m-0">
                @csrf
                @method('DELETE')
                <button type="button" onclick="konfirmasiHapusProyek()" class="inline-flex items-center px-3.5 py-1.5 bg-white border border-slate-200 text-red-600 hover:bg-red-50 hover:border-red-200 text-sm font-bold rounded-xl transition-colors shadow-sm">
                    <svg class="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex flex-col md:flex-row gap-6 md:gap-8 md:items-center">
        ...
        <!-- AREA GAMBAR (Diperbarui) -->
        <div class="w-full md:w-64 h-40 bg-slate-50 rounded-2xl flex-shrink-0 border border-slate-200 shadow-sm overflow-hidden relative flex items-center justify-center cursor-pointer hover:shadow-md transition-shadow group">
            @if($proyek->gambar)
            ...
            <!-- Tombol Edit Cepat (Ganti Gambar) saat di-hover -->
            @if(auth()->check() && auth()->user()->role === 'admin')
            <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]" onclick="toggleModal('modalEditProyek')">
        ...
        <div class="flex-1">
            ...
            <div class="mb-3">
                <div class="flex flex-col sm:flex-row sm:items-center gap-3">
                    ...
                    @php
                    $badgeClass = ''; $dotClass = '';
                    if($proyek->status == 'Berjalan') { $badgeClass = 'bg-green-100 text-green-700 border-green-200'; $dotClass = 'bg-green-600 animate-pulse'; }
            ...
            <div class="grid grid-cols-2 xl:grid-cols-4 gap-6 mt-5 border-t border-slate-100 pt-5">
                ...
                <div>
                    ...
                    <p class="text-base font-black text-green-600">Rp. {{ number_format($proyek->anggaran, 0, ',', '.') }}</p>
                </div>
                <div>
                    <p class="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-1">Timeline Proyek</p>
                    <p class="text-sm font-bold text-slate-800">{{ \Carbon\Carbon::parse($proyek->tanggal_mulai)->format('d M y') }} - {{ $proyek->estimasi_selesai ? \Carbon\Carbon::parse($proyek->estimasi_selesai)->format('d M y') : 'Belum Ditentukan' }}</p>
                </div>
                <div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
    ...
    <a href="{{ route('proyek.laporan.admin', $proyek->id) }}" class="group block bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md hover:border-blue-300 transition-all duration-300 relative overflow-hidden">
        ...
        <div class="absolute bottom-6 right-6 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
            <div class="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg></div>
        </div>
    </a>

    <!-- WIDGET 2: LAPORAN KEUANGAN (BARU) -->
    <a href="{{ route('proyek.keuangan', $proyek->id) }}" class="group block bg-white rounded-2xl border border-slate-200 p-6 shadow-sm hover:shadow-md hover:border-green-300 transition-all duration-300 relative overflow-hidden">
        <div class="absolute -right-6 -top-6 w-24 h-24 bg-green-50 rounded-full group-hover:scale-150 transition-transform duration-500 -z-10"></div>
        <div class="flex items-start gap-4">
            <div class="w-12 h-12 rounded-xl bg-green-50 text-green-600 flex items-center justify-center shrink-0 border border-green-100 group-hover:bg-green-600 group-hover:text-white transition-colors">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                </svg>
            </div>
            <div>
                <h3 class="text-base font-bold text-slate-900 mb-1 group-hover:text-green-700 transition-colors">Laporan Keuangan</h3>
                <p class="text-[13px] text-slate-500 leading-relaxed">Kelola dana termin dan pantau pengeluaran lapangan.</p>
            </div>
        </div>
        <div class="absolute bottom-6 right-6 opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
            <div class="w-8 h-8 rounded-full bg-green-50 text-green-600 flex items-center justify-center"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                </svg></div>
        </div>
    </a>

    <!-- WIDGET 3: AI FORECASTING (Segera Hadir) -->
...
<!-- ============================================== -->
<!-- PLOTTING PEKERJA & ITEM PEKERJAAN (Tetap sama persis) -->
<!-- ============================================== -->
<div class="bg-white rounded-xl shadow-sm border border-slate-100 p-6 lg:p-8 mb-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
        <div>
            <h2 class="text-lg font-bold text-slate-800 border-b-2 border-amber-500 inline-block pb-1">Tim Proyek Lapangan</h2>
            <p class="text-sm text-slate-500 mt-2">Daftar pengawas dan tukang yang bertugas di lokasi ini. Data ini akan digunakan untuk fitur Absensi harian.</p>
        </div>
        @if(auth()->check() && auth()->user()->role === 'admin')
        <button onclick="toggleModal('modalTambahPekerja')" class="inline-flex items-center px-4 py-2 bg-slate-800 text-white text-sm font-medium rounded-lg hover:bg-[#0c2340] transition-colors shadow-sm shrink-0">
        ...
        <table class="w-full text-left border-collapse min-w-[500px]">
            <thead>
                <tr class="border-b border-slate-200">
                    <th class="py-3 px-3 text-sm font-semibold text-slate-600 bg-slate-50 rounded-tl-lg">Nama Pekerja</th>
                    <th class="py-3 px-3 text-sm font-semibold text-slate-600 bg-slate-50">Jabatan</th>
                    <th class="py-3 px-3 text-sm font-semibold text-slate-600 bg-slate-50">Nomor Telepon</th>
                    @if(auth()->check() && auth()->user()->role === 'admin')
                    <th class="py-3 px-3 text-sm font-semibold text-slate-600 bg-slate-50 text-right rounded-tr-lg">Hapus dari Proyek</th>
                    @endif
                </tr>
            </thead>
            <tbody class="text-sm text-slate-700">
                ...
                <tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                    <td class="py-3 px-3 font-medium text-slate-800">{{ $pekerja->nama }}</td>
                    <td class="py-3 px-3">
                        <span class="px-2.5 py-1 {{ in_array(strtolower($pekerja->jabatan->nama_jabatan), ['mandor', 'pengawas']) ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-700' }} rounded-md text-xs font-medium">
                            {{ $pekerja->jabatan->nama_jabatan }}
                        </span>
                    </td>
                    <td class="py-3 px-3">{{ $pekerja->no_telp }}</td>
                    @if(auth()->check() && auth()->user()->role === 'admin')
                    <td class="py-3 px-3 text-right">
                        <form action="{{ route('proyek.remove', ['proyek' => $proyek->id, 'pegawai' => $pekerja->id]) }}" method="POST" onsubmit="return confirm('Keluarkan {{ $pekerja->nama }} dari proyek ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors" title="Copot Penugasan">
                ...
                @empty
                <tr>
                    <td colspan="4" class="py-8 text-center text-slate-500">Belum ada pekerja yang ditugaskan ke proyek ini. Silakan tambahkan pekerja.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
@endphp
<div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 lg:p-8 mb-6 mt-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
            <h2 class="text-lg font-semibold text-slate-900">Item Pekerjaan & Bobot</h2>
            <p class="text-sm text-slate-500 mt-1">Daftarkan tahapan pekerjaan proyek. Total bobot harus mencapai 100%.</p>
        </div>
        <div class="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100">
            ...
            <span class="text-xl font-bold {{ $totalBobot == 100 ? 'text-green-500' : 'text-slate-800' }}">
                {{ $totalBobot }}<span class="text-sm font-medium text-slate-400">/100%</span>
            </span>
        </div>
    </div>
    ...
    @if($totalBobot < 100)
        ...
        <div id="dynamic-form-container" class="space-y-4">
            <div class="flex flex-col sm:flex-row gap-4 items-end row-item group">
                ...
                <div class="w-full sm:w-32">
                    ...
                    <input type="number" name="bobot[]" step="0.01" min="0.1" max="{{ $sisaBobot }}" placeholder="Maks: {{ $sisaBobot }}" required class="w-full bg-white rounded-lg border border-slate-300 shadow-sm py-2.5 px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900 transition-all">
                </div>
                <button type="button" onclick="hapusBaris(this)" class="p-2.5 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50 border border-transparent hover:border-red-100 transition-colors hidden btn-remove" title="Hapus Baris">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                    </svg>
                </button>
        ...
        @else
        <div class="bg-green-50/50 text-green-700 p-4 rounded-xl border border-green-100 text-sm font-medium mb-8 flex items-center">
            <div class="w-8 h-8 rounded-full bg-green-100 flex items-center justify-center mr-3 shrink-0"><svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="overflow-hidden rounded-xl border border-slate-200">
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50/50 border-b border-slate-200">
                    <tr>
                        <th class="px-5 py-4 font-medium text-slate-500">Nama Tahapan</th>
                        <th class="px-5 py-4 font-medium text-slate-500 text-center">Bobot</th>
                        <th class="px-5 py-4 font-medium text-slate-500 text-center">Progres Fisik</th>
                        <th class="px-5 py-4 font-medium text-slate-500 text-right">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-5 py-4 text-center"><span class="inline-flex items-center justify-center px-2.5 py-1 rounded-md bg-slate-100 text-slate-600 font-medium text-xs">{{ $item->bobot }}%</span></td>
                        <td class="px-5 py-4 text-center"><span class="font-semibold {{ $item->progres_sekarang == 100 ? 'text-green-500' : 'text-blue-500' }}">{{ $item->progres_sekarang }}%</span></td>
                        <td class="px-5 py-4 text-right">
                            <form action="{{ route('item-pekerjaan.destroy', $item->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" onclick="return confirm('Hapus tahapan pekerjaan ini?')" title="Hapus"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                    </svg></button>
                            </form>
                        </td>
                    ...
                    <tr>
                        <td colspan="4" class="px-5 py-12 text-center">
                            <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-slate-50 mb-3"><svg class="w-6 h-6 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- 1. MODAL EDIT PROYEK (Diperbarui dengan Form Upload) -->
<div id="modalEditProyek" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm overflow-y-auto w-full h-full">
    <div class="relative top-10 mx-auto p-5 border w-full max-w-xl shadow-lg rounded-2xl bg-white mb-20">
        <div class="mt-2">
            <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                <h3 class="text-lg leading-6 font-bold text-slate-800">Edit Data Proyek</h3>
                <button type="button" onclick="toggleModal('modalEditProyek')" class="text-slate-400 hover:text-red-500 transition-colors p-1 bg-slate-50 hover:bg-red-50 rounded-lg">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form action="{{ route('proyek.update', $proyek->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <!-- UPLOAD GAMBAR PROYEK (BARU) -->
                <div class="mb-5">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Ganti Gambar Proyek <span class="text-xs font-normal text-slate-400">(Biarkan kosong jika tidak ingin mengganti)</span></label>
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Lokasi Proyek</label>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Kontak Pemilik</label>
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Anggaran / RAB (Rp)</label>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-1">Estimasi Selesai</label>
                ...
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Status Pengerjaan</label>
                ...
                <div class="flex justify-end space-x-3 pt-5 border-t border-slate-100">
                    <button type="button" onclick="toggleModal('modalEditProyek')" class="px-5 py-2.5 bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors">Batal</button>
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 text-white text-sm font-bold rounded-xl hover:bg-blue-700 transition-colors shadow-md">Simpan Perubahan</button>
                </div>
            </form>
        </div>
    </div>
...
<div id="modalTambahPekerja" class="fixed inset-0 z-50 hidden bg-slate-900/50 backdrop-blur-sm overflow-y-auto w-full h-full">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-2xl bg-white mb-20">
        <div class="mt-2">
            <div class="flex justify-between items-center mb-6 border-b border-slate-100 pb-4">
                <h3 class="text-lg font-bold text-slate-800">Tugaskan Pekerja</h3>
                <button onclick="toggleModal('modalTambahPekerja')" class="text-slate-400 hover:text-red-500 p-1 bg-slate-50 hover:bg-red-50 rounded-lg">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg>
                </button>
            ...
            <form action="{{ route('proyek.assign', $proyek->id) }}" method="POST">
                @csrf
                <div class="mb-6">
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Pilih Pekerja <span class="text-xs font-normal text-slate-500">(Belum ditugaskan)</span></label>
                    <select name="pegawai_id" required class="w-full rounded-lg border-slate-300 py-2.5 px-3 text-sm text-slate-900 focus:ring-amber-500 focus:border-amber-500 outline-none ring-1 ring-inset ring-slate-300 bg-white">
                        ...
                        <option value="{{ $tersedia->id }}">{{ $tersedia->nama }} ({{ $tersedia->jabatan->nama_jabatan }})</option>
                        @endforeach
                    </select>
                </div>
                <div class="flex justify-end space-x-3 pt-5 border-t border-slate-100">
                    <button type="button" onclick="toggleModal('modalTambahPekerja')" class="px-5 py-2.5 bg-white border border-slate-300 text-slate-700 text-sm font-bold rounded-xl hover:bg-slate-50 transition-colors">Batal</button>
                    <button type="submit" class="px-5 py-2.5 bg-slate-800 text-white text-sm font-bold rounded-xl hover:bg-[#0c2340] transition-colors shadow-md">Tugaskan ke Tim</button>
                </div>
            </form>
        </div>
    </div>
...
<!-- ============================================== -->
<!-- SCRIPT BAWAAN (SweetAlert & Modal Toggle) -->
<!-- ============================================== -->
<script>
    // JS Ubah Teks File Edit
    function previewTextEdit(input) {
        if (input.files && input.files[0]) {
            document.getElementById('file-name-edit').innerText = "File terpilih: " + input.files[0].name;
            document.getElementById('file-name-edit').classList.add('text-blue-600', 'font-bold');
        }
    }
    ...
    function tambahBaris() {
        ...
        newRow.querySelector('input[name="nama_pekerjaan[]"]').value = '';
        newRow.querySelector('input[name="bobot[]"]').value = '';
